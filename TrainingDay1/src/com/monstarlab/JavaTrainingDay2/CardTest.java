...
public class CardTest {
	...
	public static void main(String[] args) {
		...
		while (!done) {
			...
			if (dealer.getHand() == null || player.getHand().size() > 0 ) {
			...
			//while (!player.getIsOverWinningNumber() && !isDone) {

			while (!isDone) {
				
				if (player.getIsOverWinningNumber().equals(true)) {
				...
				case OPTION_BOOM:
					...
					//there is a possibility that the user will go over 25. In this case, the user loses and the game is over.
					if (newTotal.intValue() > PLAYER_LIMIT) {
						continue;
					}
					break;
					
					/*
					isDone = player.getIsOverWinningNumber();
			...
			do {
				...
				if (dealer.getIsOverWinningNumber().equals(true)) {
					System.out.println("DEALER IS OVER THE LIMIT OF " + DEALER_LIMIT);
					isDone = true;
					conclude();
					break;
				}
			...
			} while (!dealer.getIsOverWinningNumber() && !isDone);
