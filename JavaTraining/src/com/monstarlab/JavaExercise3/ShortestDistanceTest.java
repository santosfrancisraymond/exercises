...
public class ShortestDistanceTest {
	...
	public static void main(String[] args) {
		...
		// parentGraph.removeDuplicateVertices();
		graph.printEdges();

		// parentGraph.populateVertices();
		// parentGraph.printVertices();
		// parentGraph.populateNeighborEdges();
		...
		while (!done) {
			...
			default:
				...
				//graph.findShortestPath(originVertex, destinTionVertex);
				
				DijkstraAlgorithm dijkstra = new DijkstraAlgorithm(graph);
		        dijkstra.execute(graph.getVertices().get(originVertex.getVertexNumber().intValue()));

		        LinkedList<Vertex> path = dijkstra.getPath(graph.getVertices().get(destinationVertex.getVertexNumber().intValue()));
		...
		// System.out.println(LOG_SHORTEST_PATH + .toString());
